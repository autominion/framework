...
pub enum RefModification {
    Create { new_id: String, ref_name: String },
    Delete { old_id: String, ref_name: String },
    Update { old_id: String, new_id: String, ref_name: String },
}
...
pub(super) fn command_line_str(input: &str) -> IResult<&str, RefModification> {
    ...
    if token1 == ZERO_ID {
        Ok((
            input,
            RefModification::Create { new_id: token2.to_string(), ref_name: token3.to_string() },
        ))
    } else if token2 == ZERO_ID {
        Ok((
            input,
            RefModification::Delete { old_id: token1.to_string(), ref_name: token3.to_string() },
        ))
    } else {
        Ok((
            input,
