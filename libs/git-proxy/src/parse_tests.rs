...
fn test_shallow_parser_valid() {
    ...
    assert!(res.is_ok(), "Valid shallow line must be parsed successfully");
...
fn test_shallow_parser_rejects_extra_characters() {
    ...
    assert!(res.is_err(), "Extra characters after a valid object id should be rejected");
}
...
fn test_shallow_parser_invalid_objid() {
    ...
    let shallow_line = format!("shallow {}", obj_id);
    let input = pkt_line_string(&shallow_line);
    assert!(shallow(input.as_bytes()).is_err(), "Non-hex characters in object id should error");
}
...
fn test_command_line_str_create() {
    ...
    assert_eq!(
        modification,
        RefModification::Create { new_id: new_id.to_string(), ref_name: ref_name.to_string() }
    );
}
...
fn test_command_line_str_delete() {
    ...
    assert_eq!(
        modification,
        RefModification::Delete { old_id: old_id.to_string(), ref_name: ref_name.to_string() }
    );
}
...
fn test_command_line_packet() {
    ...
    assert!(remaining.is_empty(), "The parser should consume the entire packet");
    assert_eq!(
        modification,
        RefModification::Update {
            old_id: old_id.to_string(),
...
fn test_command_list_parser_success() {
    ...
    assert!(res.is_ok(), "Valid command list must be parsed successfully");
    ...
    assert_eq!(
        cmds[0],
        RefModification::Create { new_id: new_id.to_string(), ref_name: ref_name1.to_string() }
    );
...
fn test_push_cert_parser_success_standard() {
    let create_cmd =
        create_command_line("refs/heads/master", "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb");
    let input = build_push_cert_input(&["opt1"], &[&create_cmd], &["gpgsig abcdef"]);
    let res = push_cert(&input);
    assert!(res.is_ok(), "Valid push-cert input should be parsed successfully");
    let (remaining, cmds) = res.unwrap();
...
fn test_push_cert_parser_fail_wrong_header() {
    ...
    assert!(push_cert(&input).is_err(), "A wrong push-cert header must result in an error");
}
...
fn test_push_cert_parser_fail_bad_cert_version() {
    ...
    let input = s.into_bytes();
    assert!(push_cert(&input).is_err(), "An invalid certificate version should be rejected");
}
...
fn test_push_cert_parser_fail_missing_blank_line() {
    ...
    let input = s.into_bytes();
    assert!(push_cert(&input).is_err(), "Missing blank line after push-option lines should fail");
}
...
fn test_push_cert_parser_fail_missing_push_cert_end() {
    ...
    let input = s.into_bytes();
    assert!(push_cert(&input).is_err(), "Missing push-cert-end should cause an error");
}
...
fn test_push_cert_success_minimal() {
    ...
    let res = push_cert(&input);
    assert!(res.is_ok(), "A minimal push-cert input should parse successfully");
    let (remaining, cmds) = res.unwrap();
    assert!(remaining.is_empty(), "All input should be consumed");
    assert_eq!(cmds.len(), 0, "No commands should be present in minimal input");
}

#[test]
fn test_push_cert_success_multiple_commands() {
    let create_cmd =
        create_command_line("refs/heads/feature", "cccccccccccccccccccccccccccccccccccccccc");
    ...
    match &cmds[1] {
        RefModification::Update { old_id, new_id, ref_name } => {
...
fn test_push_cert_success_multiple_gpg_lines() {
    let create_cmd =
        create_command_line("refs/heads/master", "eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee");
    let input = build_push_cert_input(
        &["opt1"],
        &[&create_cmd],
    ...
    let res = push_cert(&input);
    assert!(res.is_ok(), "Multiple GPG lines should not interfere with command parsing");
    let (remaining, cmds) = res.unwrap();
    assert!(remaining.is_empty());
...
fn test_command_line_str_invalid_hex_token1() {
    ...
    assert!(command_line_str(&line).is_err(), "Invalid hex in token1 should cause an error");
}
...
fn test_command_line_with_lf_missing_lf() {
    ...
    let pkt = pkt_line_string(&cmd);
    assert!(command_line_with_lf(pkt.as_bytes()).is_err(), "Missing LF should cause an error");
}

#[test]
fn test_update_requests_parser_empty_input() {
    let input = b"";
    assert!(update_requests(input).is_err(), "Empty input should cause an error");
}
...
fn test_push_cert_parser_missing_nonce() {
    ...
    s.push_str(&pkt_line_string("push-cert-end\n"));
    let input = s.into_bytes();
    assert!(push_cert(&input).is_err(), "Missing nonce line should cause an error");
}
...
fn test_push_cert_parser_bad_pusher_line() {
    ...
    s.push_str(&pkt_line_string("push-cert-end\n"));
    let input = s.into_bytes();
    assert!(push_cert(&input).is_err(), "Bad pusher line should result in an error");
}
