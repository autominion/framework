...
async fn basic_auth_validator(
    ...
    if user == USER && pass == TOKEN {
        ...
        req.extensions_mut().insert(ProxyBehaivor {
            ...
            forward: ForwardToLocal { path: app_config.repo_path.clone() }.into(),
        });
        Ok(req)
    } else {
...
async fn main() -> std::io::Result<()> {
    ...
    let app_config = AppConfig { repo_path: repo_path.to_owned() };
    ...
    HttpServer::new(move || {
        App::new().app_data(app_config.clone()).service(scope("", basic_auth_validator))
    })
    .bind(bind_addr)?
    .run()
    .await
